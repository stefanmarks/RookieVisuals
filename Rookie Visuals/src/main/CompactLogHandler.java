...
class CompactLogHandler extends Handler 
{
    ...
    private String format(LogRecord record)
    {
        ...
        message.append(record.getLevel().getName().charAt(0)).append(String.format(" %02d:%02d:%02d.%03d\t", hour, min, sec, ms)).append(record.getLoggerName()).append(": ").append(MessageFormat.format(record.getMessage(), record.getParameters()));
